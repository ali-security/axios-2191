...
export default isHttpAdapterSupported && function httpAdapter(config) {
  return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
    ...
    if (protocol === 'data:') {
      ...
      try {
        var envOption = utils.hasOwnProp(config, 'env') ? config.env : undefined;
    ...
    const configSocketPath = own('socketPath');
    if (configSocketPath) {
      options.socketPath = configSocketPath;
    } else {
      options.socketPath = undefined; // on node <= 14 http.request doesn't guard against prototype pollution in options, so this line prevents pollution on socketPath
    ...
    req = transport.request(options, function handleResponse(res) {
      ...
      if (responseType === 'stream') {
          if (config.maxContentLength > -1) {
            ...
            const enforceMaxContentLength = async function* () {
